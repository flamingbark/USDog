...
async function main() {
  ...
  const ilks = {
    DOGE: deployment.collateral.doge.ilk,
    SHIB: deployment.collateral.shib.ilk,
  };
  ...
  // Configure dog and ilks
  console.log("Configuring Dog and ILKs...");
  try {
    const t0 = await flashLiquidator.setDog(addresses.dog);
    await t0.wait();
    const t1 = await flashLiquidator.setIlk(tokens.DOGE, ethers.encodeBytes32String(ilks.DOGE));
    await t1.wait();
    const t2 = await flashLiquidator.setIlk(tokens.SHIB, ethers.encodeBytes32String(ilks.SHIB));
    await t2.wait();
    console.log("✅ Configured Dog and ILKs");
  } catch (e) {
    console.log("⚠️ Failed to configure Dog/ILKs:", e.message);
  }
