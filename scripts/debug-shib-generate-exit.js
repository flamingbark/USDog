...
/**
 * End-to-end SHIB mint-and-exit test mirroring the frontend path:
 *  1) vat.frob(SHIB-A, u=v=w=user, dink=0, dart=wad)
 *  2) ensure vat.hope(DaiJoin)
 *  3) DaiJoin.exit(user, wad)
 *
 * Usage:
 *   AMOUNT=0.000001 npx hardhat run scripts/debug-shib-generate-exit.js --network bsc
 */
async function main() {
  ...
  // Pre-state
  const urn0 = await vat.urns(SHIB_ILK, USER);
  ...
  // 1) frob mint only
  console.log("\n1) vat.frob(SHIB-A, dink=0, dart=wad) ...");
  const tx1 = await vat.frob(SHIB_ILK, USER, USER, USER, 0, wad);
  ...
  // Post-state
  const urn1 = await vat.urns(SHIB_ILK, USER);
  ...
  console.log("\n✅ SHIB generate-and-exit end-to-end complete.");
}

main().catch((e) => {
  console.error("❌ debug-shib-generate-exit error:", e);
  process.exit(1);
});